...
async def register(user_data: UserCreate, background_tasks: BackgroundTasks):
    ...
    # Send email notification if user registers as instructor
    if user.role == "instructor":
        ...
        <body>
            ...
            <p>A new instructor has registered on LearnHub:</p>
            <ul>
                <li><strong>Name:</strong> {user.name}</li>
                <li><strong>Email:</strong> {user.email}</li>
                <li><strong>Registration Date:</strong> {user_doc['created_at']}</li>
                <li><strong>User ID:</strong> {user.id}</li>
            </ul>
            <p>Please review this instructor application and take appropriate action if needed.</p>
        </body>
        </html>
        """
        # Send email to admin email (you can customize this to your admin email)
...
async def apply_instructor(bio: str, current_user: User = Depends(get_current_user), background_tasks: BackgroundTasks):
    ...
    # Send email notification about instructor application
    subject = "New Instructor Application"
    content = f"""
    <html>
    <body>
        <h2>New Instructor Application</h2>
        <p>A user has applied to become an instructor on LearnHub:</p>
        <ul>
            <li><strong>Name:</strong> {current_user.name}</li>
            <li><strong>Email:</strong> {current_user.email}</li>
            <li><strong>Application Date:</strong> {doc['created_at']}</li>
            <li><strong>User ID:</strong> {current_user.id}</li>
            <li><strong>Bio:</strong> {bio}</li>
        </ul>
        <p>Please review this instructor application and approve or reject it as appropriate.</p>
    </body>
    </html>
    """
...
async def approve_instructor(instructor_id: str, approved: bool, current_user: User = Depends(get_current_user), background_tasks: BackgroundTasks):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin only")

    # Get instructor details before update
    instructor_doc = await db.instructors.find_one({"id": instructor_id})
    if not instructor_doc:
        raise HTTPException(status_code=404, detail="Instructor not found")

    # Get user details
    user_doc = await db.users.find_one({"id": instructor_doc['user_id']})
    if not user_doc:
    ...
    # Send email notification to instructor about the decision
    status_text = "APPROVED" if approved else "REJECTED"
    subject = f"Instructor Application {status_text}"
    content = f"""
    <html>
    <body>
        <h2>Instructor Application Status Update</h2>
        <p>Your instructor application on LearnHub has been <strong>{status_text}</strong>.</p>
        <ul>
            <li><strong>Name:</strong> {user_doc['name']}</li>
            <li><strong>Email:</strong> {user_doc['email']}</li>
            <li><strong>Decision:</strong> {status_text}</li>
            <li><strong>Decision Date:</strong> {datetime.now(timezone.utc).isoformat()}</li>
        </ul>
        <p>{'Congratulations! You can now create courses and start teaching.' if approved else 'Unfortunately, your application was not approved at this time. You may reapply in the future.'}</p>
    </body>
    </html>
    """
    background_tasks.add_task(send_email, user_doc['email'], subject, content)
...
async def get_pending_courses(current_user: User = Depends(get_current_user)):
    ...
    for course in courses:
        instructor = await db.instructors.find_one({"user_id": course['instructor_id']}, {"_id": 0})
        if instructor:
            user = await db.users.find_one({"id": instructor['user_id']}, {"_id": 0, "password": 0})
...
async def download_certificate(certificate_id: str):
    ...
    temp_path = f"/tmp/certificate_{certificate_id}.pdf"
